...
- **Web UI**：FastAPI + WebSocket 实时 Web 界面，支持多浏览器标签同时连接，Mermaid 图表渲染
...
- **Plan 模式**：`Shift+Tab` 切换，结构化输出实施计划，支持审批流程（`enter_plan_mode` + `submit_plan`）
- **Skill 调用**：`invoke_skill` 工具化加载 Skill 模板
- **Stop Reason 处理**：主循环无迭代上限，由 LLM 自然结束；检测 max_tokens 截断并提示用户
...
- **上下文管理**：根据模型上下文窗口自动计算压缩阈值，长对话自动摘要压缩
