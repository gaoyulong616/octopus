...
- `workdir_base`（默认 `null`）：WebUI 每会话默认工作目录根路径。设置为目录后，各会话 cwd 为 `<workdir_base>/<user_id>/<session_id>`，目录自动创建。`null` 时使用 `os.getcwd()`（服务器进程 cwd），保持原有的全局共享行为。
...
- **Web 会话右键菜单**：`web/static/app.js` 的 `initUI` 中创建 `.ctx-menu` 浮层。菜单项：重命名（行内编辑，替换 `<span>` 为 `<input>`，Enter 保存，Esc/Blur 取消，调 `PATCH /api/sessions/{id}`）/ 删除（`.ctx-popup` 确认卡片，调 `delete_session` WS action，先 detach bridge 再删文件）/ 断开会话（调 `disconnect_session` WS action，`_archive_and_detach` 保存进度后移除 bridge）/ 恢复会话。菜单项根据 `activeSessionIds`/`sessionId` 自动禁用（当前会话/已在池中时"恢复"禁用；不在池中时"断开"禁用）。**仅点击图标（`i.ti-message`，亮绿色时）触发断开**，点击栏目其他位置（文字/空白）不触发断开。
- **WebUI 每会话独立工作目录**：`compute_session_cwd(user_id, session_id)` 在 `web/agent_bridge.py` 实现，生成 `<workdir_base>/<user_id>/<session_id>` 路径，目录不存在自动创建。`config.py` 的 `workdir_base` 配置项控制根路径（默认 `null`=os.getcwd()）。三个创建路径（初始连接、new_session、_handle_resume）都会调用 `bridge.agent_state.set_cwd(session_cwd)`，确保各会话 cwd 互不干扰。
