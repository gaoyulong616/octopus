...
| `agent.py` | Agent 主循环：流式 API 调用、7 种事件类型（EVT_STREAM/THINKING/TOOL_CALL/TOOL_RESULT/RESPONSE/PROGRESS/ERROR）、重试退避、thinking 块、prompt cache、stop reason 处理、服务端工具探测（`_probe_server_tools`）、`_stream_with_retry` 直接迭代 stream 事件实时发射 ServerToolUseBlock/WebSearchToolResultBlock、`_emit_server_search_result`/`_emit_server_fetch_result` 格式化结果 |
...
- **会话序列化**：`_serialize_content()` 处理 8 种 block 类型（text/tool_use/tool_result/thinking/server_tool_use/web_search_tool_result/web_fetch_tool_result），`_deserialize_content()` 还原时跳过 thinking。服务端工具 block 按 API 原始字段保存，加载时透传回 API
- **安全**：`tools/security.py` 提供 SSRF 防护、敏感路径检测；`config.py` 维护 `dangerous_commands` 列表
- **服务端工具**：`agent.py` 启动时 `_probe_server_tools()` 探测 API 提供商是否支持 `web_search_20260209`/`web_fetch_20260209`，支持则使用服务端版本（零额外往返、SPA 渲染），不支持则降级到客户端 `run_web_search()`/`run_web_fetch()`。探测结果按 `(base_url, api_key)` 缓存，切换提供商自动重新探测。`_stream_with_retry` 直接迭代 stream 事件，通过 `ParsedContentBlockStopEvent` 实时发射 ServerToolUseBlock（EVT_TOOL_CALL）和 WebSearchToolResultBlock（EVT_TOOL_RESULT），保证工具调用与文本输出的顺序一致。block 处理循环对这三种 server 类型跳过。前端 `pendingToolCalls[]` FIFO 匹配，历史渲染后清空防泄漏
