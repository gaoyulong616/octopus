...
- 三档权限模式：`Plan`（完全只读）/ `Accept Edits`（默认，编辑自动 + 命令确认）/ `Auto`（全自动 YOLO）。`state["mode"]` ∈ `{"plan", "accept-edits", "auto"}`；`confirm_fn` 返回 `(approved, reason, source)`，source 区分 "user"（用户主动拒绝）/ "system"（规则/模式拒绝）
