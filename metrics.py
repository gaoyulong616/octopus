...
def format_stats(filters: dict | None = None) -> str:
    ...
    # cache_write 是建立缓存（首次写入），不算命中也不算未命中
    cacheable_total = agg["cache_read"] + agg["input"]
    cache_hit_rate = (agg["cache_read"] / cacheable_total * 100) if cacheable_total > 0 else 0.0

    lines = [
        f"调用次数: {agg['calls']}",
        f"Tokens: input={agg['input']:,}  output={agg['output']:,}  "
        f"cache_read={agg['cache_read']:,}  cache_write={agg['cache_write']:,}",
        f"缓存命中率: {cache_hit_rate:.1f}%  （命中率 = cache_read / (cache_read + input)）",
